...
const PatientTrialMatchView = ({ trialItem, patientContext, onClose }) => {
  ...
  // --- Data Extraction (Safely access nested data) ---
    ...
    <div className="p-4 lg:p-6 bg-gray-50 min-h-screen">
      ...
      <div className="flex justify-between items-center mb-4">
        ...
        {onClose && (
            <button 
                onClick={onClose}
                className="ml-4 p-2 text-gray-500 rounded-full hover:bg-gray-200"
                title="Close View"
            >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 lg:gap-6">
        ...
        {/* --- Panel 1: Patient & Trial Summary (Left Column) --- */}        
        ...
        {/* --- Panel 2: Eligibility Deep Dive (Center Column) --- */}       
                    ...
                    {unclearCriteria.map((item, index) => (
                      <li key={index} className="text-xs text-gray-700 bg-yellow-50 p-2 rounded border border-yellow-200">
                        <div>
                          ...
                          {item.reasoning && 
                            <span className="text-yellow-800 italic ml-1">- {item.reasoning}</span>
                          }
                        </div>
                        <div className="flex justify-start items-center gap-2 mt-1.5 pt-1.5 border-t border-yellow-100"> 
                          <button 
                            onClick={() => handleDraftMessageClick(item)} 
                            className="flex items-center gap-1 px-1.5 py-0.5 text-xs font-medium text-blue-700 bg-blue-100 rounded hover:bg-blue-200"
                          ...
                          <button 
                            onClick={() => handleFlagReviewClick(item)}
                            className="flex items-center gap-1 px-1.5 py-0.5 text-xs font-medium text-orange-700 bg-orange-100 rounded hover:bg-orange-200"
        ...
        {/* --- Panel 3: Agent Insights & Actions (Right Column) --- */}        
        <div className="lg:col-span-3 bg-white p-4 rounded-lg border border-gray-200 shadow-sm">
